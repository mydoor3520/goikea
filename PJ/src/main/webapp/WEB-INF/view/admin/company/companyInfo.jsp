	...
	<div class="contents_area" style="text-align: left;">
		<form action="companyEditer" method="post"> <!-- enctype="multipart/form-data" -->
			<table class="table-hover" style="padding: 5px 0 5px 0; width: 1080;">
				<tbody>
					<c:forEach var="company" items="${detailCompany}">
						<tr>
							<td width="20%"></td>
							<th width="20%">판매자</th>
							<td>
						...
						<tr>
							<td width="20%"></td>
							<th>담당자 이메일</th>
							<td><input type='text' name="m_email" required value=<%=request.getAttribute("m_email1")%>>@ <input
							id="dns_sel" list="dns" name="m_email_dns" placeholder="직접 입력도 가능"
							autocomplete="off" value=<%=request.getAttribute("m_email2")%>> 
							<datalist id="dns">
						...
						<tr>
							...
							onkeyPress="InpuOnlyNumber(this);" value="${company.c_post}">
						...
						<tr>
							...
							<td><input type="text" id="address1" class="input-control" name="c_addr1" 
								maxlength="35" size="50" required value="${company.c_addr1}" > <br>
							<input type="text" id="address2" class="input-control" name="c_addr2" 
								maxlength="35" size="50" required value="${company.c_addr2}"> 상세주소
						</td>
						</tr>
					</c:forEach>
				</tbody>
